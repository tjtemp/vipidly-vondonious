/**
 * Created by joo on 17. 5. 18.
 */

// ml-core

// image-ml-core
// 기능 01 : Job 추가 할 때 Job list 데이터 테이블에 row 추가

// Controller panel 에 ctr1-job-table1 을 선언합니다.
$(document).ready(function() {
    var table = $('#ctrl-job-table1').DataTable( {
        columnDefs: [ {
            orderable: false,
...
// 기능 01
// 방법 1. 버튼 클릭시 javascript 단에서 table row append 해주기
// TODO: jobform에서 바로 serialization을 해주는 방법을 못찾겠다.
// TODO: beforeSend로 달았으므로 에러가 발생했을 때 ajax error 에서 테이블에서 지워주어야 한다. 아니면 에러상태 표시.
...
//               alert(jobpriority + jobname + username);
    ...
    $('.delete_yes_button').on('click', function(){
        ...
        var job_pks = $.map(table.rows('.selected').data(), function (item) {
            return item[1]
        });
        console.log(job_pks);
        ...
        $.ajax({
            type: 'POST',
            url: '',
            data: {'job_pks[]': job_pks, 'ajax_differer': ajax_delete_differer, 'delete_job': 1},
            success: function(data){
            ...
            $('#req').on('click', function(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        $('#answer').append("<br/>");
                        $('#answer').append(data.consoles);
                        $('#answer').append(data.model_consoles);
                        $('#answer-ml-pre-report').append(data.plots);
                        $('#answer-ml-model-report').append(data.model_plots);
                        $('#answer-ml-report-stack').append(data.report_stack);
                        //data_plot3d = data._3dplots;
                        //console.log(data_plot3d);
                        //$('#answer-ml-3d-report').append(data._3dplots);
                    },
